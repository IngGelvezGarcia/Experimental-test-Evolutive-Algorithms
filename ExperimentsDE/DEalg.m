%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [FVr_bestmem,FVr_oa,I_iter,I_nfeval,S_bestval] = DEalg(fname,FVr_bound,S_struct)
%                    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [FVr_bestmem,FVr_oa,I_iter,I_nfeval,S_bestval] = DEalg(fname,FVr_bound,S_struct)
%
% Adapted version from: https://github.com/sriki18/MDEpBX-Matlab/blob/master/deopt.m
% Under GNU General Public License
% Original author: Rainer Storn, Ken Price, Arnold Neumaier, Jim Van Zandt
...
I_D          = length(FVr_bound(:,1));
FVr_minbound = FVr_bound(:,1)';
FVr_maxbound = FVr_bound(:,2)';

ShowIter = S_struct.ShowIter;
...
%-----Select which vectors are allowed to enter the new population------------
  for k=1:I_NP
      ...
      ValFobj = feval(fname,FM_ui(k,:));   % check cost of competitor
...
%----Output section----------------------------------------------------------
  ...
  if (I_refresh > 0) && ShowIter
     if ((rem(I_iter,I_refresh) == 0) || I_iter == 1)
       ...
       if (I_plotting == 1)
          %PlotIt(FVr_bestmem,I_iter,S_struct); %No esta definida la
